...
export class P5SketchComponent implements OnInit {
  ...
  private createCanvas(): void {
    this.p5Instance = new p5((p: p5) => {
      ...
      p.mousePressed = () => {
        p.fill(
          p.floor(100 - p.noise(p.mouseX * rez, p.mouseY * rez) * 100) +
            colorStart,
          100,
          100,
          100
      ...
      p.draw = () => {
        ...
        for (let i = 0; i <= window.innerWidth; i += size) {
          for (let j = 0; j <= window.innerHeight; j += size) {
            ...
            const drawCircle = () =>
              p.ellipse(i + size / 2, j + size / 2, size, size);
            const drawTriangle = () =>
              p.triangle(i + size / 2, j, i + size, j + size, i, j + size);
            const drawLine = () => p.line(i, j, i + size, j + size);
            const fillMain = (num: number) =>
              p.fill(p.floor(num * 100) + colorStart, 100, 100, 100);
            const fillOff = (num: number) =>
              p.fill(p.floor(100 - num * 100) + colorStart, 100, 100, 100);
            const boundsScale = 2;
            const withinBounds = () =>
              p.mouseX > i - (boundsScale - 1) * size &&
              p.mouseX < i + boundsScale * size &&
              p.mouseY > j - (boundsScale - 1) * size &&
              p.mouseY < j + boundsScale * size;
            const conditionalFill = (num: number) =>
              withinBounds() ? fillMain(num) : fillOff(num);
